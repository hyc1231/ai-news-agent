...
# 生成合成内容标识：简报正文由大模型生成，按《人工智能生成合成内容标识办法》
# （2025-09-01 施行）的口径，在正文末尾显式标注这是 AI 生成内容。
#
# 单机自用本无标识义务（该办法约束的是「向公众提供服务的提供者」）；
# 加这一行的意义在于该办法第十条：使用者把生成合成内容发布到网络传播平台时
# 应当主动声明并标识 —— 有了这一行，用户转发简报时就天然带上了标识。
AI_NOTICE_ZH = "> 本简报由 AI 自动生成，仅供个人参考。"
AI_NOTICE_EN = "> This digest was generated by AI and is for personal reference only."
...
def _with_notice(digest: str, language: str) -> str:
    """
    给简报尾部统一补上 AI 生成标识。

    收口在这里而不是写进各个分支：模型正常输出与 _fallback_digest 兜底
    两条路径都必须带上标识，分散写迟早漏掉一条。
    """
    notice = AI_NOTICE_EN if language == "en" else AI_NOTICE_ZH
